...
Streamable HTTP connections always use the experimental Rust MCP client under the hood, so expect occasional rough edges. When a server supports OAuth, run `codex mcp login <server-name>` to authenticate.
